...
/** Opens the File menu (custom DOM titlebar) and clicks the given item id. */
export async function clickFileMenuItem(
  page: Page,
  itemId: string,
): Promise<void> {
  await page.getByTestId("title-menu-file").click();
  const item = page.getByTestId(`menu-${itemId}`);
...
export async function createRootNote(
  ...
  await clickFileMenuItem(page, "new-root-note");
