...
`manual-strong` _before_ arming the mock (prevents a background auto-download),
...
reports the new version and reaches a working UI — so the updated _binary_ is
proven, not just the swap. When the fetched artifact _is_ the base, it falls
