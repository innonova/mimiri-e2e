...
flowchart TD
    ...
    F --> W["native-dialog-win.ts<br/>UI Automation + SendKeys"]
```
...
## Windows — UI Automation + SendKeys
...
It then foregrounds the picker and drives it with SendKeys: type the path,
Enter (navigate), Enter (accept).
